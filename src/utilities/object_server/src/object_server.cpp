...
bool ObjectServer::initialize(ros::NodeHandle& nh)
{
  ...
  int obstacle_num;
  if (!cc::load(ns + "description/obstacle_num", obstacle_num))
    return false;
  
  int target_num;
  if (!cc::load(ns + "description/target_num", target_num))
    return false;
...
{
  cc::Scalar period;
  cc::LinearPosition x0_map;
  cc::LinearPosition n_map; 
  cc::LinearPosition d_map;
  for(size_t i = 0; i < trajectories.size(); ++i)
  {
    cc::Scalar r_fac = 1.0;
    std::vector<cc::Scalar> param;
    std::string id = name + std::to_string(i);

    if (!cc::load(ns + "description/" + id + "/period", period))
    {
      ROS_ERROR("Error loading period");
      return false;
    }
    if (!cc::load(ns + "description/" + id + "/x0_map", x0_map))
    {
      ROS_ERROR("Error loading x0_map");
      return false;
    }
    if (!cc::load(ns + "description/" + id + "/n_map", n_map))
    {
      ROS_ERROR("Error loading n_map");
      return false;
    }
    if (!cc::load(ns + "description/" + id + "/d_map", d_map))
    {
      ROS_ERROR("Error loading d_map");
      return false;
    }
    cc::load(ns + "description/" + id + "/r_fac", r_fac);
    cc::load(ns + "description/" + id + "/param", param);

    if(param.size() == 3)
    {
      trajectories[i] =
        std::make_shared<cc::ParametricPlanarCartesianTrajectory>
          (period, x0_map, n_map, x0_map + d_map, param[0], param[1], param[2]);
    }
    else if(param.size() == 4)
    {
      trajectories[i] =
        std::make_shared<cc::ParametricCartesianTrajectory>
          (period, x0_map, r_fac, param[0], param[1], param[2]);
    }
    else
    {
      trajectories[i] =
        std::make_shared<cc::CircularCartesianTrajectory>
...
bool ObjectServer::addTargets()
{
  ...
  std::vector<cc::Vector3> colors(5);
  colors[0] << 1,0,0;
  colors[1] << 0,1,0;
  colors[2] << 0,0,1;
  colors[3] << 1,1,0;
